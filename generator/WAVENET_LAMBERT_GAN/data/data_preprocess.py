...
def data_preprocess(
    ...
    # Additional initial values for log_returns reconstruction (per window, per ticker)
    output_initial_high = []
    output_initial_low = []
    ...
    if normalization_method == "lambert":
        ...
        if feature_method == "log_returns":
            _pv_feat_names = ['log_ret_close', 'log_ret_open',
                              'log_ret_high', 'log_ret_low', 'log_ret_volume']
        else:
            _pv_feat_names = ['close_return', 'open_close_return',
                              'high_close_ratio_return', 'low_close_ratio_return', 'volume']

        for _ticker in tqdm(ticker_list, desc="Lambert fit"):
            _td = ticker_data_dict[_ticker].copy()
            _td = _td.set_index('date').reindex(full_data_index).reset_index()
            _td.reset_index(drop=True, inplace=True)

            _pv = _td[PV_features].copy().bfill().ffill()
            if feature_method == "log_returns":
                _feat_df, _initial_vals = process_data_into_log_returns(_pv)
            else:
                _feat_df, _pv_close, _pv_open, _pv_caj = process_data_into_feature(_pv)
            _feat_arr = _feat_df[_pv_feat_names].values  # (T, 5)

            # 3-stage Lambert per feature: StandardScaler → Gaussianize → StandardScaler
            ...
            if feature_method == "log_returns":
                ticker_features_precomputed[_ticker] = {
                    'features': _transformed,
                }
            else:
                ticker_features_precomputed[_ticker] = {
                    'features': _transformed,
        ...
        # Print summary
        for _ticker in ticker_list[:3]:
            _deltas = [p[1].delta_[0] if p[1] is not None else 0.0
                       for p in lambert_fit_params[_ticker]]
            print(f"  {_ticker} deltas: {[f'{d:.4f}' for d in _deltas]}")
        if len(ticker_list) > 3:
            print(f"  ... ({len(ticker_list) - 3} more tickers)")
    ...
    for i in tqdm(range(max_seq_len, no - max_seq_len, step)):
        ...
        for ticker in ticker_list:
            ...
            if feature_method == "log_returns":
                processed_pv_features, _initial_vals = process_data_into_log_returns(pv_data_ticker_copy)
                processed_pv_features_name = ['log_ret_close', 'log_ret_open', 'log_ret_high', 'log_ret_low', 'log_ret_volume']
                pv_close = _initial_vals['close']
                pv_open = _initial_vals['open']
                pv_caj = None
            else:
            ...
            if normalization_method == "lambert":
                # Use pre-computed Lambert W Gaussianized features (already ~N(0,1))
                precomp = ticker_features_precomputed[ticker]
                curr_pv_sample_normalized = precomp['features'][i:i + max_seq_len]
                history_pv_sample_normalized = precomp['features'][i - max_seq_len:i]
            else:
                # Rolling z-score normalization with history window
                history_mean = history_pv_sample.mean(axis=0)
                history_std = history_pv_sample.std(axis=0)
            ...
            # Collect initial OHLCV values at window start for log_returns reconstruction
            if feature_method == "log_returns":
                output_initial_high.append(pv_data_ticker['high'].iloc[i])
                output_initial_low.append(pv_data_ticker['low'].iloc[i])
                output_initial_volume.append(pv_data_ticker['volume'].iloc[i])
    ...
    return output_data, output_macro_data, output_history_data, output_mask_data, time, max_seq_len, ticker_list, lambert_fit_params, log_returns_initials
...
def process_data_into_log_returns(df):
    ...
    Each channel's log return is self-referential: log(price[t+1] / price[t]).
    This eliminates cross-channel error propagation during reconstruction.

    Parameters:
        df (pd.DataFrame): DataFrame with columns ["close", "open", "high", "low", "volume"]

    Returns:
        pd.DataFrame: 5 log return features
        dict: initial values for each channel (first row), needed for reconstruction
    """
    ...
    # Forward fill NaN in last row (from shift(-1)), then backfill any remaining
...
def revert_log_returns_to_data(df_features, initial_values):
    """Revert forward log returns back to OHLCV with independent per-channel reconstruction.

    Applies OHLC bar clamping: high >= max(open, close), low <= min(open, close).

    Parameters:
        df_features (pd.DataFrame or np.ndarray): 5 columns of log returns
            [log_ret_close, log_ret_open, log_ret_high, log_ret_low, log_ret_volume]
        initial_values (dict): keys 'close', 'open', 'high', 'low', 'volume'

    Returns:
        pd.DataFrame: Reconstructed OHLCV data
    """
    ...
    # Independent cumulative reconstruction per channel
    close  = initial_values['close']  * np.exp(np.cumsum(lr[:, 0]))
    open_  = initial_values['open']   * np.exp(np.cumsum(lr[:, 1]))
    high   = initial_values['high']   * np.exp(np.cumsum(lr[:, 2]))
    low    = initial_values['low']    * np.exp(np.cumsum(lr[:, 3]))
    volume = initial_values['volume'] * np.exp(np.cumsum(lr[:, 4]))
