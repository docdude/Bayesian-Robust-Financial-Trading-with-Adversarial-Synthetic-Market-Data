...
class EnvironmentRET(gym.Env):
    ...
    def __init__(self,
                 ...
                 level="day",
                 position_lowerbound=0,
                 position_upperbound=None,
                 ):
        ...
        self.level = level
        self.position_lowerbound = position_lowerbound
        self.position_upperbound = position_upperbound
        ...
        if if_norm:
            ...
            if self.mode == "train":
                ...
                for df in self.dataset.stocks_df:
                    ...
                    print("df start date: ", df.index[0])
                    print("df end date: ", df.index[-1])
    ...
    def reset(self, **kwargs):
        ...
        self.position = 0
        self.previous_stock_proportion = 0
        self.discount = 1.0
        self.value = self.initial_amount
        self.total_return = 0
        self.total_profit = 0
        self.action = self.actions[self.hold_on_action]
    ...
    def trade(self, previous_stock_proportion, stock_proportion, price):
        if previous_stock_proportion == stock_proportion:
    ...
    def eval_buy_position(self, price):
        ...
        all_in_position = int(np.floor(self.cash / price / (1 + self.transaction_cost_pct)))
        if self.position_upperbound is None:
            return all_in_position
        else:
            return max(self.position_upperbound - int(self.position), all_in_position)

    def eval_sell_position(self):
        # evaluate sell position
        return int(self.position) - self.position_lowerbound
