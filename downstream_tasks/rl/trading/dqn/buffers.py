...
def build_storage_numpy(shape, type):
    if type.startswith("int32"):
        type = np.int32
    elif type.startswith("float32"):
        type = np.float32
...
class ReservoirReplayBuffer:
    def __init__(self, 
        ...
        self.num_envs = self.transition_shape["states"]["shape"][0]
        for name in self.transition:
            ...
            self.storage[name] = build_storage_numpy(shape, type)
            
        self.idx = 0
        self.cur_size = 0
            
    def update(self, items: Tuple[Any, ...]):
        if self.idx < self.buffer_size:
            insert_index = self.idx
        else:
            insert_index = np.random.randint(0, self.idx)
        if insert_index < self.buffer_size:
            for i in range(len(items)):
                self.storage[self.transition[i]][insert_index] = items[i]
        self.idx = self.idx + 1
...
class ReplayBuffer:  # for off-policy
    ...
    def update(self, items: Tuple[Any, ...]):
        ...
        items = [torch.Tensor(item).unsqueeze(0).to(self.device) for item in items]
